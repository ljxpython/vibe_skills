---
description: "生成测试计划（本次+E2E+核心逻辑+每接口覆盖），写入 /docs/test-plan.md"
---
...
3. 读取 `@.claude/skills/test-coverage/references/test-template.md` 作为模板。
4. 读取 `@.claude/skills/test-coverage/references/api-scan.md`，优先运行 `python3 .claude/skills/test-coverage/scripts/scan_apis.py --json` 扫描接口；失败则按规则手工扫描。
